...
use crate::{
    App, Player, Position, RunState,
    component::OtherLevelPosition,
    generate::{
        map::{Map, MapOptions},
        spawn::{
            spawn_npc_captain, spawn_npc_merchant, spawn_player, spawn_weighted_item,
            spawn_weighted_monster,
        },
    },
};
...
/// them both here.
pub fn switch_floor(app: &mut App, next_index: u32) -> RunState {
...
pub fn thaw_floor(index: u32, world: &mut World) {
    ...
    for (entity, other_level_position) in (&entities, &other_level_positions).join() {
        ...
        if other_level_position.index != index {
            continue;
        }
        positions
            .insert(
